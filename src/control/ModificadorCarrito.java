...
public class ModificadorCarrito extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			case "editarCarrito":
				...
				break;
			case "eliminarAlimentoCarrito": 
				eliminarAlimentoCarrito(request,response); 
				break;
			case "confirmarOrden":
	...
	private void verCarrito(HttpServletRequest request, HttpServletResponse response) throws SQLException, ServletException, IOException{
		...
		if(session!= null) {
			...
			alimentos = carritoDAO.getAlimentos(idCliente); 
			if (alimentos == null) {
	...
	private void editarCarrito(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, SQLException {
		HttpSession session = request.getSession(false); 
		if(session != null) {
			Cliente cliente = (Cliente)session.getAttribute("cliente"); 
			int idAlimento = Integer.parseInt(request.getParameter("alimento"));
			int idCarrito = Integer.parseInt(request.getParameter("idC")); 
			int cantidad = Integer.parseInt(request.getParameter("cantidad"));
			
			
			carritoDAO.editarCantidad(idAlimento, cantidad,idCarrito);
	...
	private void eliminarAlimentoCarrito(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, SQLException {
		HttpSession session = request.getSession(false); 
		if(session != null) {
			Cliente cliente = (Cliente)session.getAttribute("cliente"); 
			int idAlimento = Integer.parseInt(request.getParameter("alimento"));
			...
			carritoDAO.eliminarAlimentoCarrito(idAlimento, idCarrito);
